...
// equivalent.
func ToStorage(cluster, namespace, resultName, resultID, name string, r *pb.Record, conf *conf.ConfigFile) (*db.Record, error) {
	...
	if r.GetData().GetType() == v1alpha2.TaskRunLogRecordType {
		data, err := toTaskRunLogStorage(FormatParentDB(cluster, namespace), resultName, name, r, conf)
		if err != nil {
			return nil, err
		}
		dbr.Data = data
