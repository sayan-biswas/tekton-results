...
import (
	...
	pb "github.com/tektoncd/results/pkg/server/db/pagination/proto/internal_go_proto"
...
func NewBatcher(want int) *Batcher {
	return &Batcher{
		want:  want,
		min:   minPageSize,
		max:   maxPageSize,
...
func PageStart(token, filter string) (string, error) {
	...
	tokenName, tokenFilter, err := DecodeToken(token)
	if err != nil {
