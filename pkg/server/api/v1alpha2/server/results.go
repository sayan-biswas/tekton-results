...
import (
	...
	"github.com/tektoncd/results/pkg/server/kcp"
	"log"
...
func (s *Server) CreateResult(ctx context.Context, req *rpb.CreateResultRequest) (*rpb.Result, error) {
	...
	parent, _, err := result.ParseName(res.GetName())
	if err != nil {
	...
	store, err := result.ToStorage(res)
	if err != nil {
		return nil, err
	}
...
func (s *Server) GetResult(ctx context.Context, req *rpb.GetResultRequest) (*rpb.Result, error) {
	...
	parent, err = kcp.PhysicalClusterNamespaceName(kcp.GetLogicalClusterName(ctx), parent)
	if err != nil {
		return nil, err
	}
...
func (s *Server) DeleteResult(ctx context.Context, req *rpb.DeleteResultRequest) (*empty.Empty, error) {
	...
	get := s.db.WithContext(ctx).
		Where(&models.Result{Parent: parent, Name: name}).
		First(r)
	...
	// Delete the result.
	delete := s.db.WithContext(ctx).Delete(&models.Result{}, r)
...
func (s *Server) ListResults(ctx context.Context, req *rpb.ListResultsRequest) (*rpb.ListResultsResponse, error) {
	...
	if err := s.auth.Check(ctx, cluster, parent, auth.ResourceResults, auth.PermissionList); err != nil {
		return nil, err
	}

	parent, err := kcp.PhysicalClusterNamespaceName(cluster, parent)
	if err != nil {
		return nil, err
	}
...
func (s *Server) getFilteredPaginatedSortedResults(ctx context.Context, parent string, start string, pageSize int, prg cel.Program, sortOrder string) ([]*rpb.Result, error) {
...
func getResultByParentName(gdb *gorm.DB, parent, name string) (*models.Result, error) {
	r := &models.Result{}
	if err := errors.Wrap(gdb.Where(&models.Result{Parent: parent, Name: name}).First(r).Error); err != nil {
